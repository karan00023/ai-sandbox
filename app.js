const CONFIG = {
  ...
  macroTargets: {
    calories: [1800, 2000], protein: [180, 200], fat: [60, 70], carbs: [100, 150],
    highCarbDays: ["Monday", "Wednesday", "Friday"], highCarbRange: [150, 180]
  },
  workoutSchedule: {
...
const WORKOUTS = {
  Monday: [
    ["Barbell Back Squats", "4 x 8-10", "2-3 min"],
    ["Romanian Deadlifts (RDLs)", "4 x 10", "2-3 min"],
    ["Leg Press", "3 x 12-15", "90 sec"],
    ["Walking Lunges", "3 x 12 per leg", "90 sec"],
    ["Leg Curls", "3 x 12", "60 sec"],
    ["Calf Raises", "4 x 15-20", "60 sec"],
    ["HIIT Treadmill Sprints", "30s sprint / 90s walk x 7", "15 min"],
  ],
  Tuesday: [
    ["Barbell Bench Press", "4 x 8-10", "2-3 min"],
    ["Incline Dumbbell Press", "4 x 10-12", "2 min"],
    ["Cable Flyes", "3 x 12-15", "90 sec"],
    ["Dips (chest focus)", "3 x 10-12", "90 sec"],
    ["Close-Grip Bench Press", "3 x 10", "2 min"],
    ["Rope Pushdowns", "3 x 12-15", "60 sec"],
    ["Overhead Dumbbell Extension", "3 x 12", "60 sec"],
    ["Bike Intervals", "30s hard / 90s easy x 7", "15 min"],
  ],
  Wednesday: [
    ["Deadlifts", "4 x 6-8", "3 min"],
    ["Pull-ups or Lat Pulldowns", "4 x 8-12", "2 min"],
    ["Barbell Rows", "4 x 8-10", "2 min"],
    ["Seated Cable Rows", "3 x 12", "90 sec"],
    ["Face Pulls", "3 x 15", "60 sec"],
    ["Barbell Curls", "3 x 10", "90 sec"],
    ["Hammer Curls", "3 x 12", "60 sec"],
    ["Cable Curls", "3 x 15", "60 sec"],
    ["Rowing Intervals", "30s max / 90s easy x 7", "15 min"],
  ],
  Thursday: [
    ["Complete Rest", "Recovery focus", "-"],
    ["Light Walk", "30-45 min / 10k steps", "-"],
    ["Yoga / Stretching", "20-30 min", "-"],
  ],
  Friday: [
    ["Overhead Press", "4 x 8-10", "2-3 min"],
    ["Lateral Raises", "4 x 12-15", "90 sec"],
    ["Front Raises", "3 x 12", "90 sec"],
    ["Reverse Flyes", "3 x 15", "60 sec"],
    ["Dumbbell Shrugs", "3 x 12", "90 sec"],
    ["Hanging Leg Raises", "3 x 12-15", "60 sec"],
    ["Cable Crunches", "3 x 15-20", "60 sec"],
    ["Planks", "3 x 60 sec", "60 sec"],
    ["Russian Twists", "3 x 20 total", "60 sec"],
    ["Battle Rope / Sprint Intervals", "30s max / 90s rest x 7", "15 min"],
  ],
  Saturday: [
    ["Dumbbell Goblet Squats", "3 rounds x 15", "90 sec"],
    ["Push-ups", "3 rounds x 15-20", "90 sec"],
    ["Dumbbell Romanian Deadlifts", "3 rounds x 12", "90 sec"],
    ["Pull-ups (or band-assisted)", "3 rounds x 8-12", "90 sec"],
    ["Dumbbell Lunges", "3 rounds x 12 per leg", "90 sec"],
    ["Dumbbell Shoulder Press", "3 rounds x 12", "90 sec"],
    ["Dumbbell Rows", "3 rounds x 12 per arm", "90 sec"],
    ["Plank", "3 rounds x 60 sec", "90 sec"],
    ["HIIT Finisher", "Burpees + Jump Rope + Mountain Climbers", "15-20 min"],
  ],
  Sunday: [
    ["Walk", "45-60 min", "-"],
    ["Light Yoga", "30-45 min", "-"],
    ["Swimming", "30-45 min", "-"],
...
function mergeState(incoming = {}) { Object.assign(state, structuredClone(defaultState), incoming); }
...
async function loadState() {
  mergeState(JSON.parse(localStorage.getItem(KEY) || "{}"));
  try {
    const r = await fetch("/api/state");
    if (!r.ok) return setSyncStatus("Saved locally (cloud sync unavailable)", false);
    const payload = await r.json();
    if (payload.state && Object.keys(payload.state).length) { mergeState(payload.state); saveLocal(); }
    setSyncStatus("SQLite sync connected", true);
  } catch {
    setSyncStatus("Saved locally (cloud sync unavailable)", false);
  }
}
...
function renderDashboard() {
  ...
  document.getElementById("dashboard-metrics").innerHTML = [["Days clean", daysFromStart()], ["Week", `${currentWeek()}/8`], ["Days to 90", Math.max(0, daysBetween(isoToday, CONFIG.sobriety90Date))], ["Weight", latest.weight ?? "-"], ["Lost", startWeight ? (startWeight - (latest.weight || startWeight)).toFixed(1) : "-"], ["Waist", latest.waist ?? "-"], ["Workout", CONFIG.workoutSchedule[dayName()].type], ["Protein", `${todayMacro.protein}g/${CONFIG.macroTargets.protein[0]}g`]].map(([k,v]) => `<div class="metric"><strong>${v}</strong><span>${k}</span></div>`).join("");
}
...
function renderMacros() {
  const m = state.nutritionLog[isoToday] || { calories: 0, protein: 0, fat: 0, carbs: 0 };
